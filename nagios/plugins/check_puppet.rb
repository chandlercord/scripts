...
if File.size?(summaryfile)
else
	puts "WARN: Puppet is not installed"
	exit 1
end

file_age = Time.now - File.mtime(summaryfile)
...
unless failures
    if enabled_only && enabled == false
        puts "OK: Puppet is currently disabled, not alerting.  Last run #{time_since_last_run} seconds ago with #{failcount} failures | puppet_last_ran=#{time_since_last_run};#{warn};#{crit}, failure_count=#{failcount}"
        exit 0
    end

    if time_since_last_run >= crit
        puts "CRITICAL: Puppet last ran #{time_since_last_run} seconds ago, expected < #{crit} | puppet_last_ran=#{time_since_last_run};#{warn};#{crit}, failure_count=#{failcount}"
		if file_age > 3600
			exec 'sudo /sbin/service puppet stop >/tmp/blah; sudo /bin/rm /var/lib/puppet/state/puppetdlock; sudo /usr/bin/puppet agent --test >/tmp/blah 2>&1 &'
		end
        exit 2

    elsif time_since_last_run >= warn
        puts "WARNING: Puppet last ran #{time_since_last_run} seconds ago, expected < #{warn} | puppet_last_ran=#{time_since_last_run};#{warn};#{crit}, failure_count=#{failcount}"
		if file_age > 3600
			exec 'sudo /sbin/service puppet stop >/tmp/blah; sudo /bin/rm /var/lib/puppet/state/puppetdlock; sudo /usr/bin/puppet agent --test >/tmp/blah 2>&1 &'
		end
        exit 1
        ...
        if enabled
            puts "OK: Puppet is currently enabled, last run #{time_since_last_run} seconds ago with #{failcount} failures | puppet_last_ran=#{time_since_last_run};#{warn};#{crit}, failure_count=#{failcount}"
        else
            puts "OK: Puppet is currently disabled, last run #{time_since_last_run} seconds ago with #{failcount} failures | puppet_last_ran=#{time_since_last_run};#{warn};#{crit}, failure_count=#{failcount}"
        end
    ...
    if failcount >= crit
        puts "CRITICAL: Puppet last ran had #{failcount} failures, expected < #{crit} | puppet_last_ran=#{time_since_last_run};#{warn};#{crit}, failure_count=#{failcount}"
        exit 2

    elsif failcount >= warn
        puts "WARNING: Puppet last ran had #{failcount} failures, expected < #{warn} | puppet_last_ran=#{time_since_last_run};#{warn};#{crit}, failure_count=#{failcount}"
        exit 1
