...
my $statusfile = "./status.dat.enabled";

#To and from email addresses
my $to = "chandlercord\@gmail.com";
my $from = "nagios\@myserver.com";
my $emailaddress = $to;

#Nagios URL
my $nagiosurl = "http://myserver.com/nagios3/";

#Turn on verbose messages
my $VERBOSE="0";

#Usage message
my $USAGE = "Please specify global or notifications:\n$0 <global> or $0 <notifications>\n";
...
if ($type =~ m/notifications/i) {
	$status="notifications";
} elsif ($type =~ m/global/i) {
	$status="global";
} else {
	print "$USAGE";
	exit 1
...
if ($type =~ m/notifications/i) {
	...
	while (<INPUT>) {
		...
		if ($block eq "programstatus") {
			...
			print $notifications . "\n";
			if ( $notifications == 0) {
				print "Notifications disabled." if $VERBOSE eq "1" && $block eq "programstatus";
...
EOF
				...
				print "Notifications enabled." if $VERBOSE eq "1" && $block eq "programstatus";
				exit 1
			}
		}
	}
