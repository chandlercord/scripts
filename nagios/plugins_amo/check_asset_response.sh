...
pageload(){
	...
	if [ ! -n "$CONTENT_TYPE" ]; then
		...
		GOGET=`$CURL -i -o ${WGETOUTCKSUM}.output -s -XPOST -H "$CONTENT_TYPE" -d "<assetRequest xmlns=\"com:tango:multimedia:assetcatalog:jaxb:v1\" version=\"1.0\"><assetId>1</assetId><locale>en_US</locale><contentVersionMax>1.0</contentVersionMax></assetRequest>" $URL`
	elif [ -n "$PING" ]; then
		...
		GOGET=`$CURL -i -o ${WGETOUTCKSUM}.output -s -XPOST -H "$CONTENT_TYPE" -d "<assetCatalogRequest xmlns=\"com:tango:multimedia:assetcatalog:jaxb:v1\" version=\"1.0\"><assetType>ALL</assetType><assetState>RELEASE</assetState><platformCode>1</platformCode><locale>en_US</locale><maxResultsLimit>100</maxResultsLimit><contentVersionMax>1.0</contentVersionMax><attributeFilter xmlns=\"com:tango:multimedia:assetcatalog:jaxb:v1\"><assetType>ANIMATION_PACK</assetType><name>ENGINE</name><value>CAFE</value></attributeFilter><attributeFilter xmlns=\"com:tango:multimedia:assetcatalog:jaxb:v1\"><assetType>ANIMATION_PACK</assetType><name>CAPABILITY</name><value>CINEMATIC</value></attributeFilter></assetCatalogRequest>" $URL`
...
output(){
	if [ ! -n "$CONTENT_TYPE" ]; then
		...
		$ECHO "$STATUS - $RESPCODE - $OUTMSG - `/bin/cat $WGETOUTCKSUM.output | $GREP "xml version" | $SED 's/.*\(checksum.*"\).*/\1/'`|Response="$TIMEDIFF"ms;"$WARN";"$CRIT";0"
		#$ECHO "$STATUS - $RESPCODE - $OUTMSG - `/bin/cat $WGETOUTCKSUM.output | $GREP "xml version"`|Response="$TIMEDIFF"ms;"$WARN";"$CRIT";0"
	elif [ -n "$ASSET" ]; then
		RESPCODE=`$GREP HTTP ${WGETOUTCKSUM}.output | $SED 's/.*\([0-9][0-9][0-9]\).*/\1/g'`
		$ECHO "$STATUS - $RESPCODE - $OUTMSG - `/bin/cat $WGETOUTCKSUM.output | $GREP "xml version" | $SED 's/.*<\(contentChecksum\)>\(.*\)..contentC.*/\1=\2/'`|Response="$TIMEDIFF"ms;"$WARN";"$CRIT";0"
		#$ECHO "$STATUS - $RESPCODE - $OUTMSG - `/bin/cat $WGETOUTCKSUM.output | $GREP "xml version"`|Response="$TIMEDIFF"ms;"$WARN";"$CRIT";0"
	elif [ -n "$PING" ]; then
		RESPONSE=`$GREP "xml " ${WGETOUTCKSUM}.output | $SED 's/.*\(HOSTNAME\)\">/\1=/;s/<.*\(TOMCAT_VERSION\)\">/. \1=/;s/<.*\(APP_NAME\)\">/. \1=/;s/<.*\(SUCCESS\)\">/. \1=/;s/<.*\(USED_MEMORY\)\">/. \1=/;s/<.*\(UPTIME\)\">/. \1=/;s/<.*\(APP_VERSION\)\">/. \1=/;s/<.*\(TOTAL_MEMORY\)\">/. \1=/;s/<.*\(JVM_VERSION\)\">/. \1=/;s/<.*//g'`
		RESPCODE=`$GREP HTTP ${WGETOUTCKSUM}.output | $SED 's/.*\([0-9][0-9][0-9]\).*/\1/g'`
...
### Main
# Input variables
while getopts w:c:h:p:s:pta option
	do case "$option" in
		w) WARN=$OPTARG;;
		c) CRIT=$OPTARG;;
		h) NODE=$OPTARG;;
		p) PORT=$OPTARG;;
		s) SLASH=$OPTARG;;
		p) PING="TRUE"; CONTENT_TYPE='Content-Type: text/xml';;
		a) ASSET="TRUE"; CONTENT_TYPE='Content-Type: text/xml';;
		t) TYPE="TRUE"; CONTENT_TYPE='Content-Type: text/xml';;
