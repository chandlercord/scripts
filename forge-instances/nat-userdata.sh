...
PUBIP="$(wget http://169.254.169.254/latest/meta-data/public-ipv4 -qO -)"
CORENATS=WWWWWW
...
sudo yum -y install openswan s3cmd
#sudo curl -L https://www.opscode.com/chef/install.sh | sudo bash
sudo mkdir -p /etc/chef/

#Set up tunnels:
if [ -e "/etc/ipsec.conf" ]; then echo "include /etc/ipsec.d/*.conf">>/etc/ipsec.conf; fi
mkdir -p  /etc/ipsec.d/
for NAT in CORENATS; do
echo "conn $TAG-to-core
  type=tunnel
	authby=secret
	left=%defaultroute
	leftid=$PUBIP
	...
	pfs=yes
	auto=start">/etc/ipsec.d/$TAG-to-core.conf
echo "$PUBIP $NAT"': "HdbT.29tAxeSR-_gm4KLx77_pcg_3ZR8"' >/etc/ipsec.d/$TAG-to-core.secrets
done
