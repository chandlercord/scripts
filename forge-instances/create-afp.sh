...
for SUBNETID in $SUBNETIDLIST; do
	...
	NAME="${REGIONID}-${ENVIRONMENTID}-${APPCODE}-${INSTANCENUM}"
	TMPFILE=$(mktemp)
	...
	i=""
	INSTANCEID=$(../bin/ec2-run-instances ami-927613a2 --user-data-file $TMPFILE -g $SGROUP -n 1 -k livemagic_ops -t ${SIZE} --subnet ${SUBNETID} |grep INSTANCE|awk '{print $2}')
