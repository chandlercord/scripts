...
REGIONID="usw2"
SIZE="m1.small"
SUBNETIDLIST=$(../bin/ec2-describe-subnets -F "tag:environment=${ENVIRONMENTID}" -F "tag:region=${REGIONID}" -F "tag:app=${APPID}" -F "tag:az=a" --hide-tags |awk '{print $2}')
echo "Searching for $APPID security group"
SGROUP=$(../bin/ec2-describe-group -F "tag:app=${APPID}" -F "tag:environment=${ENVIRONMENTID}" -F "tag:region=${REGIONID}" -F "tag:az=a" --hide-tags|grep GROUP | awk '{print $2}')
for SUBNETID in $SUBNETIDLIST; do
	...
	i=""
	INSTANCEID=$(../bin/ec2-run-instances ami-927613a2 -n 1 -g ${SGROUP} -k livemagic_ops -t ${SIZE} --subnet ${SUBNETID} |grep INSTANCE|awk '{print $2}')
